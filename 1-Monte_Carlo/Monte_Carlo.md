...
- 训练窗口 $L\in\{32,64,128\}$；
- $L_{\mathrm{parent}}=512$ 被定义为本阶段固定背景，不宣称为严格无限体极限。
...
|---|---:|---:|---|
| `train` | 8 | 512 | 在线随机裁取 $32/64/128$ 窗口 |
| `val` | 4 | 512 | 固定小窗口验证与模型选择 |
| `reference_a` | 4 | 512 | 固定 $32/64/128$ MC 参考 |
...
1. 每个 batch 先从 $\{32,64,128\}$ 均匀选择一个尺寸；
...
- `val` 固定生成 $32/64/128$ crop；
- `reference_a` 和 `reference_b` 固定生成 $32/64/128$ crop；
- 计算 crop 关联时只统计窗口内有效点对，不做周期 wrap；
- 每个 crop 的主要关联分析限制在 $r\le W/4$，减少开放窗口边缘的低计数影响。

同一父组态可以贡献多个训练 crop，但不能因此增加统计上的独立样本数。验证和参考分析默认每个 parent、每个尺寸只取一个固定 crop；若需要多个 crop，bootstrap 时必须保留 parent 层级。

### 4.8 固定背景一致性确认

本阶段不再要求 $L_{\mathrm{parent}}=1024$ 与 $512$ 的 crop 分布相同。主数据验收后，对 $W\in\{32,64,128\}$ 比较 `reference_a` 和 `reference_b` 的开放窗口关联曲线，并用链级平衡标签置换检验判断两个独立 split 是否可交换。

固定背景门禁同时要求合并链的平均磁化与 $0$ 在三个链级标准误内相容。该门禁只支持“$L=512$ 固定背景下的局部样本一致”，不支持无限体外推结论；同一父组态产生的多个 crop 仍不得当作独立样本。
